 ...
 SELECT pav.id,
   ...
   FROM ( SELECT p.id,
           ...
           FROM post p
             ...
             CROSS JOIN ( SELECT user_.id,
                    user_.name,
                    user_.preferred_username,
                    user_.password_encrypted,
                    user_.email,
                    user_.avatar,
                    user_.admin,
                    user_.banned,
                    ...
                    user_.show_nsfw,
                    user_.theme,
                    user_.default_sort_type,
                    user_.default_listing_type,
                    user_.lang,
                    user_.show_avatars,
                    user_.send_notifications_to_email,
                    ...
                    user_.private_key,
                    user_.public_key,
                    user_.last_refreshed_at,
                    user_.sitemod,
                    user_.banner,
                    user_.has_2fa,
                    user_.inbox_disabled
                   FROM user_
                  WHERE user_.banned = false) me
             ...
             LEFT JOIN post_like pl ON me.id = pl.user_id AND p.id = pl.post_id
          WHERE p.id > 230000) pav
UNION ALL
 SELECT p.id,
    p.name,
    p.url,
